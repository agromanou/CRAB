        ...
        while not should_break:
            try:
                if model =='gpt4':
                    ...
                    predictions = ask_gpt3(prompts=prompts, prompt_system=prompt_system, openai_key=openai_key, test=False)
                else:
...
def main(openai_key, 
    ...
    if selected_prompt:
        dataset[prompt_name] = dataset.apply(lambda x: selected_prompt.format(x['article_a'], x['article_b'],
                                                                              x['event_a'], x['event_b']), axis=1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Ask GPT to generate pairwise causality scores.')
    ...
    parser.add_argument('--prompt-name', 
                        required=False, 
                        default='pairwise_score_w_context',
                        help='')
    
    parser.add_argument('--data-path', 
                        required=False, 
                        default='data/pairwise_causality.jsonl',
                        help='')

    args = parser.parse_args()
    prompt_name = args.prompt_name
    
    destination_file = 'data/CRAB_pairwise_pred_{}_gpt4.json'.format(prompt_name)
